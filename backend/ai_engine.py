...
- 각 과목 하위에 구체적인 단원(목차) 리스트를 생성하세요. 단, 교재의 단순한 '단원명'이나 '페이지 수'로 분할하지 말고, '성취해야 할 핵심 목표나 행동(Action)' 단위로 `unit_name`을 작성하세요.
- **매우 중요**: 각 단원(`units`)에는 해당 단원의 학습 범위를 가상으로 추정하여 `start_page`와 `end_page`를 반드시 정수로 포함하세요. 단원들 간의 페이지는 서로 겹치거나 비지 않도록 연속적으로 구성되어야 합니다. (이 페이지 데이터가 일일 진도 배분 및 UI 표시에 사용됩니다.)
  ...
  {{
    ...
    "subjects": [
      {{
        ...
        "weight_percent": 40,
        "units": [
          {{"unit_name": "시의 화자가 느끼는 감정 3가지 찾아내어 설명하기", "weight_percent": 30, "start_page": 1, "end_page": 15}},
          {{"unit_name": "소설의 시점을 파악하고 주인공의 심리 변화 요약하기", "weight_percent": 70, "start_page": 16, "end_page": 40}}
        ]
      }},
      {{
        "subject_name": "수학",
        "weight_percent": 60,
        "units": [
          {{"unit_name": "일차방정식의 개념을 이해하고 동생에게 설명하듯 말해보기", "weight_percent": 50, "start_page": 1, "end_page": 20}},
          {{"unit_name": "함수의 그래프를 직접 그리고 특징 3가지 찾아내기", "weight_percent": 50, "start_page": 21, "end_page": 45}}
        ]
      }}
    ]
  }}
